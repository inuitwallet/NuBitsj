...
import com.nubits.nubitsj.store.*;
...
/**
 * A command line tool for manipulating wallets and working with Nubits.
 */
public class WalletTool {
    ...
    private static org.nubits.protocols.payments.Protos.PaymentRequest paymentRequest;
    ...
    public static class Condition {
        ...
        public boolean matchNubitss(Coin comparison) {
            try {
    ...
    private static void sendPaymentRequest(String location, boolean verifyPki) {
        if (location.startsWith("http") || location.startsWith("nubits")) {
            ...
            try {
                paymentRequest = org.nubits.protocols.payments.Protos.PaymentRequest.newBuilder().mergeFrom(stream).build();
            } catch(IOException e) {
    ...
    private static void wait(WaitForEnum waitFor) throws BlockStoreException {
        ...
        switch (waitFor) {
            ...
            case BALANCE:
                ...
                if (condition.matchNubitss(wallet.getBalance(Wallet.BalanceType.ESTIMATED))) {
                    latch.countDown();
                    break;
                }
                wallet.addEventListener(new AbstractWalletEventListener() {
                    ...
                    public synchronized void onChange() {
                        ...
                        if (condition.matchNubitss(balance)) {
    ...
    private static void deleteKey() {
        ...
        if (pubkey != null) {
            ...
            try {
                ...
                System.err.println(addr + " does not parse as a Nubits address of the right network parameters.");
                return;
            }
        }
        if (key == null) {
