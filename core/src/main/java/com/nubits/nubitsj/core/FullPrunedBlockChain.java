...
import com.nubits.nubitsj.script.Script.VerifyFlag;
import com.nubits.nubitsj.store.BlockStoreException;
import com.nubits.nubitsj.store.FullPrunedBlockStore;
 ...
 * Nubits system, with the downside being a larg cost in system resources. Fully verifying means all unspent transaction
...
public class FullPrunedBlockChain extends AbstractBlockChain {    
    ...
    public FullPrunedBlockChain(NetworkParameters params, List<BlockChainListener> listeners,
        ...
        super(params, listeners, blockStore, validHashStore);
    ...
    private static class Verifier implements Callable<VerificationException> {
        ...
        public Verifier(final Transaction tx, final List<Script> prevOutScripts, final Set<Script.VerifyFlag> verifyFlags) {
    ...
    protected TransactionOutputChanges connectTransactions(int height, Block block)
        ...
        long sigOps = 0;
        final Set<Script.VerifyFlag> verifyFlags = EnumSet.noneOf(Script.VerifyFlag.class);
        ...
        try {
            if (!params.isCheckpoint(height)) {
                ...
                for (Transaction tx : block.transactions) {
                    ...
                    if (verifyFlags.contains(Script.VerifyFlag.P2SH)) // We already check non-BIP16 sigops in Block.verifyTransactions(true)
            ...
            for (final Transaction tx : block.transactions) {
                ...
                if (!isCoinBase) {
                    ...
                    for (int index = 0; index < tx.getInputs().size(); index++) {
                        ...
                        if (verifyFlags.contains(Script.VerifyFlag.P2SH)) {
