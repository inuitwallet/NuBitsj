...
import com.nubits.nubitsj.script.ScriptChunk;
import com.nubits.nubitsj.signers.LocalTransactionSigner;
import com.nubits.nubitsj.signers.MissingSigResolutionSigner;
...
import com.nubits.nubitsj.utils.BaseTaggableObject;
...
import com.nubits.nubitsj.protocols.payments.Protos.PaymentDetails;
 ...
 *
 * <p>Wallets can be serialized using either Java serialization - this is not compatible across versions of nubitsj,
...
public class Wallet extends BaseTaggableObject implements Serializable, BlockChainListener, PeerFilterProvider, KeyBag, TransactionBag {
     ...
     * key, after loading a wallet produced by previous versions of nubitsj. If the wallet is encrypted the key
     ...
     * @throws com.nubits.nubitsj.core.ECKey.MissingPrivateKeyException if the seed is unavailable (watching wallet)
     */
    ...
    public void receivePending(Transaction tx, @Nullable List<Transaction> dependencies, boolean overrideIsRelevant) throws VerificationException {
        ...
        try {
            ...
            // between pools.
            EnumSet<WalletTransaction.Pool> containingPools = getContainingPools(tx);
    ...
    public boolean isTransactionRelevant(Transaction tx) throws ScriptException {
        lock.lock();
        try {
            if (tx.getCoinType() != Transaction.NUBITS)
                return false;
    ...
    private void receive(Transaction tx, StoredBlock block, BlockChain.NewBlockType blockType,
        ...
        if (bestChain) {
            ...
            if (wasPending) {
                ...
                addWalletTransaction(WalletTransaction.Pool.PENDING, tx);
                log.info("  ->pending");
            } else {
    ...
    public void notifyNewBestBlock(StoredBlock block) throws VerificationException {
        ...
        try {
            ...
            for (Transaction tx : transactions) {
                ...
                } else if (tx.getConfidence().getConfidenceType() == TransactionConfidence.ConfidenceType.BUILDING) {
    ...
    private void processTxFromBestChain(Transaction tx, boolean forceAddToPool) throws VerificationException {
        ...
        if (isDeadCoinbase) {
            ...
            // we could. Nubits nodes on the network have deleted the dependent transactions from their mempools
        ...
        if (hasOutputsToMe) {
            ...
            if (tx.isEveryOwnedOutputSpent(this)) {
                ...
                addWalletTransaction(WalletTransaction.Pool.SPENT, tx);
            } else {
                log.info("  tx {} ->unspent", tx.getHashAsString());
                addWalletTransaction(WalletTransaction.Pool.UNSPENT, tx);
            }
        ...
        for (TransactionInput input : tx.getInputs()) {
            ...
            if (result == TransactionInput.ConnectionResult.ALREADY_SPENT) {
                if (fromChain) {
                    ...
                    log.warn("{}: {}", tx.getHash(), Utils.HEX.encode(tx.unsafeNubitsSerialize()));
                    Transaction other = input.getConnectedOutput().getSpentBy().getParentTransaction();
                    log.warn("{}: {}", other.getHash(), Utils.HEX.encode(tx.unsafeNubitsSerialize()));
                }
    ...
    private void killTx(@Nullable Transaction overridingTx, List<Transaction> killedTx) {
        ...
        while (!work.isEmpty()) {
            ...
            addWalletTransaction(WalletTransaction.Pool.DEAD, tx);
    ...
    public String toString(boolean includePrivateKeys, boolean includeTransactions, boolean includeExtensions,
        ...
        try {
            ...
            builder.append(String.format("Wallet containing %s NBT (available: %s NBT) in:%n",
     ...
     * of {@link com.nubits.nubitsj.core.ECKey#getCreationTimeSeconds()}. This can return zero if at least one key does
    ...
    public static class SendResult {
        /** The Nubits transaction message that moves the money. */
    ...
    public static class SendRequest {
         ...
         * (its value is ignored and set to {@link com.nubits.nubitsj.core.Wallet#getBalance()} - the fees required
        ...
        public SendRequest fromPaymentDetails(com.nubits.nubitsj.protocols.payments.Protos.PaymentDetails paymentDetails) {
     ...
     *
     * @param address The Nubits address to send the money to.
    ...
    public void completeTx(SendRequest req) throws InsufficientMoneyException, IOException {
        ...
        try {
            ...
            if (!req.emptyWallet) {
                ...
                feeCalculation = calculateFee(req, value, originalInputs, candidates);
    ...
    public void reorganize(StoredBlock splitPoint, List<StoredBlock> oldBlocks, List<StoredBlock> newBlocks) throws VerificationException {
        ...
        try {
            ...
            for (Transaction tx : oldChainTxns) {
                ...
                tx.getConfidence().setConfidenceType(TransactionConfidence.ConfidenceType.PENDING);  // Wipe height/depth/work data.
                confidenceChanged.put(tx, TransactionConfidence.Listener.ChangeReason.TYPE);
                addWalletTransaction(WalletTransaction.Pool.PENDING, tx);
                updateForSpends(tx, false);
            }
                ...
                for (int i = 0; i < tx.getOutputs().size(); i++) {
                    ...
                    try {
                        if (isTxOutputBloomFilterable(out)) {
                            ...
                            filter.insert(outPoint.nubitsSerialize());
                        }
                    } catch (ScriptException e) {
    ...
    public FeeCalculation calculateFee(SendRequest req, Coin value, List<TransactionInput> originalInputs,
        ...
        // 1) No effect
        // 2) Causes the change to be thrown away due to it being below the minimum output
        // If we get the last one we keep note of what the inputs looked like at the time and try to
        ...
        if (selection2 != null) {
            if (lowestFee == null || selection2.valueGathered.compareTo(lowestFee) < 0) {
     ...
     * sending transactions to the Nubits network by default. Some sendCoins methods let you specify a broadcaster
    ...
    public void setTransactionBroadcaster(@Nullable TransactionBroadcaster broadcaster) {
        ...
        for (Transaction tx : toBroadcast) {
            checkState(tx.getConfidence().getConfidenceType() == TransactionConfidence.ConfidenceType.PENDING);
     ...
     * @throws DeterministicUpgradeRequiresPassword if key rotation requires the users password.
     */
    ...
    private Transaction rekeyOneBatch(long timeSecs, @Nullable KeyParameter aesKey, List<Transaction> others, boolean sign) throws IOException {
        ...
        try {
            ...
            checkState(rekeyTx.nubitsSerialize().length < Transaction.MAX_STANDARD_TX_SIZE);
            return rekeyTx;
     ...
     * {@link PeerFilterProvider} interface and generally should not be used directly by apps.
