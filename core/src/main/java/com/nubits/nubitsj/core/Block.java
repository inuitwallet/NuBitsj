...
import static com.nubits.nubitsj.core.Utils.doubleDigest;
import static com.nubits.nubitsj.core.Utils.doubleDigestTwoBuffers;
 ...
 * <a href="http://www.nubits.org/nubits.pdf">the Nubits technical paper</a> for
...
public class Block extends Message {
     ...
     * upgrade everyone to change this, so Nubits can continue to grow. For now it exists as an anti-DoS measure to
    ...
    /** Constructs a block object from the Nubits wire format. */
    ...
    /**
     * Contruct a block object from the Nubits wire format.
    ...
    private void parseTransactions() throws ProtocolException {
        ...
        if (payload.length == cursor || payload.length - 1 == cursor) { // Include the -1 case for the null byte which is required for nubits
    ...
    public int getOptimalEncodingMessageSize() {
        ...
        if (optimalEncodingMessageSize != 0)
            return optimalEncodingMessageSize;
        optimalEncodingMessageSize = nubitsSerialize().length;
        return optimalEncodingMessageSize;
    }
    ...
    /** Returns the version of the block data structure as defined by the Nubits protocol. */
     ...
     * {@link com.nubits.nubitsj.core.Block#getDifficultyTargetAsInteger()}. Note that this is <b>not</b> the same as
     * the difficulty value reported by the Nubits "getdifficulty" RPC that you may see on various block explorers.
