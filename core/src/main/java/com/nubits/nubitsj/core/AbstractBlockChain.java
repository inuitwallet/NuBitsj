...
import com.nubits.nubitsj.store.BlockStoreException;
import com.nubits.nubitsj.store.H2FullPrunedBlockStore;
import com.nubits.nubitsj.store.ValidHashStore;
import com.nubits.nubitsj.utils.ListenerRegistration;
import com.nubits.nubitsj.utils.Threading;
 ...
 * the lightweight SPV mode, a {@link com.nubits.nubitsj.store.SPVBlockStore} is the right choice.</p>
 *
 ...
 * {@link H2FullPrunedBlockStore} implements full verification, which is equivalent to the
...
public abstract class AbstractBlockChain {
     ...
     *
     * Following this one down to the genesis block produces the story of the economy from the creation of Nubits
        ...
        for (Transaction tx : transactions) {
            try {
                if (listener.isTransactionRelevant(tx)) {
                    ...
                    if (clone)
                        tx = new Transaction(tx.params, tx.nubitsSerialize());
