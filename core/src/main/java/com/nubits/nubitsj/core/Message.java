...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using both the Nubits proprietary serialization
...
public abstract class Message implements Serializable {
     ...
     * @param protocolVersion Nubits protocol version.
    ...
    private void selfCheck(byte[] payload, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = nubitsSerialize();
    ...
    // These methods handle the serialization/deserialization using the custom Nubits protocol.
    ...
    /**
     * Should only used by NubitsSerializer for cached checksum
     *
     * @return the checksum
     */
    byte[] getChecksum() {
    ...
    /**
     * Should only used by NubitsSerializer for caching checksum
     *
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafeNubitsSerialize()}, which is safe to mutate.
     ...
     */
    public byte[] nubitsSerialize() {
        byte[] payload = unsafeNubitsSerialize();
    ...
    /**
     * Serialize this message to a byte array that conforms to the nubits wire protocol.
     * <br/>
     ...
     */
    public byte[] unsafeNubitsSerialize() {
        ...
        try {
            nubitsSerializeToStream(stream);
        } catch (IOException e) {
        ...
        if (parseRetain) {
            ...
            // which means another call to nubitsSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the nubits wire format.
     *
     * @param stream
     * @throws IOException
     */
    final public void nubitsSerialize(OutputStream stream) throws IOException {
    ...
    /**
     * Serializes this message to the provided stream. If you just want the raw payload use nubitsSerialize().
     */
    void nubitsSerializeToStream(OutputStream stream) throws IOException {
        log.error("Error: {} class has not implemented nubitsSerializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so NubitsSerializer can avoid 2 instanceof checks + a casting.
     */
