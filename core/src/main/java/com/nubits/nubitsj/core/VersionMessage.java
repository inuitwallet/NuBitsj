...
public class VersionMessage extends Message {
    ...
    public static final String NubitsJ_VERSION = "0.14.5";
    ...
    public VersionMessage(NetworkParameters params, int newBestHeight) {
        ...
        try {
            ...
            myAddr = new PeerAddress(InetAddress.getByAddress(localhost), params.getPort(), PeerAddress.INC_TIME_PROTOCOL);
            theirAddr = new PeerAddress(InetAddress.getByAddress(localhost), params.getPort(), PeerAddress.INC_TIME_PROTOCOL);
    ...
    public void parse() throws ProtocolException {
        ...
        myAddr = new PeerAddress(params, payload, cursor, PeerAddress.INC_TIME_PROTOCOL);
        cursor += myAddr.getMessageSize();
        theirAddr = new PeerAddress(params, payload, cursor, PeerAddress.INC_TIME_PROTOCOL);
    ...
    @Override
    public void nubitsSerializeToStream(OutputStream buf) throws IOException {
        ...
        try {
            // My address.
            myAddr.nubitsSerialize(buf);
            // Their address.
            theirAddr.nubitsSerialize(buf);
        } catch (UnknownHostException e) {
     ...
     * name:version pairs separated by slashes in the form of a path. For example a typical subVer field for NubitsJ
     * users might look like "/NubitsJ:0.4-SNAPSHOT/MultiBit:1.2/" where libraries come further to the left.<p>
     *
     ...
     * set of "/NubitsJ:1.0/MultiBit:1.0(Windows)/. Therefore the / ( and ) characters are reserved in all these
