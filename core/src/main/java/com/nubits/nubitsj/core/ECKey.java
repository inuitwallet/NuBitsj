...
import com.nubits.nubitsj.crypto.NativeSecp256k1;
 ...
 *
 * <p>ECKey also provides access to Nubits-Qt compatible text message signing, as accessible via the UI or JSON-RPC.
...
public class ECKey implements EncryptableItem, Serializable {
    ...
    /** The parameters of the secp256k1 curve that Nubits uses. */
    ...
    /**
     * Construct an ECKey from an ASN.1 encoded private key. These are produced by OpenSSL and stored by the Nubits
    ...
    /**
     * Output this ECKey as an ASN.1 encoded private key, as understood by OpenSSL or used by the Nubits reference
     ...
     * how ECDSA signatures are represented when embedded in other data structures in the Nubits protocol. The raw
    ...
    public static class ECDSASignature {
         ...
         * the same message. However, we dislike the ability to modify the bits of a Nubits transaction after it's
    ...
    /**
     * Signs the given hash and returns the R and S components as BigIntegers. In the Nubits protocol, they are
     * usually encoded using DER format, so you want {@link com.nubits.nubitsj.core.ECKey.ECDSASignature#toASN1()}
    ...
    /**
     * Signs the given hash and returns the R and S components as BigIntegers. In the Nubits protocol, they are
     * usually encoded using DER format, so you want {@link com.nubits.nubitsj.core.ECKey.ECDSASignature#encodeToDER()}
    ...
    /**
     * Given an arbitrary piece of text and a Nubits-format message signature encoded in base64, returns an ECKey
     * containing the public key that was used to sign it. This can then be compared to the expected public key to
     * determine if the signature was correct. These sorts of signatures are compatible with the Nubits-Qt/nubitsd
     * format generated by signmessage/verifymessage RPCs and GUI menu options. They are intended for humans to verify
     * their communications with each other, hence the base64 format and the fact that the input is text.
     *
     * @param message Some piece of human readable text.
     * @param signatureBase64 The Nubits-format message signature in base64
     ...
     * the {@link com.nubits.nubitsj.core.DumpedPrivateKey#toString()} method to get the string.
     *
