...
import static com.nubits.nubitsj.script.ScriptOpCodes.*;
import static com.google.common.base.Preconditions.*;

/**
 * <p>Programs embedded inside transactions that control redemption of payments.</p>
 *
 * <p>Nubits transactions don't specify what they do directly. Instead <a href="https://en.nubits.it/wiki/Script">a
...
public class Script {
    ...
    public boolean isSentToRawPubKey() {
        return chunks.size() == 2 && chunks.get(1).equalsOpCode(ScriptOpCodes.OP_CHECKSIG) &&
    ...
    public byte[] getPubKey() throws ScriptException {
        ...
        if (chunk0data != null && chunk0data.length > 2 && chunk1data != null && chunk1data.length > 2) {
            ...
            return chunk1data;
        } else if (chunk1.equalsOpCode(ScriptOpCodes.OP_CHECKSIG) && chunk0data != null && chunk0data.length > 2) {
     ...
     * The concept of a "from address" isn't well defined in Nubits and you should not assume the sender of a
    ...
    private int findSigInRedeem(byte[] signatureBytes, Sha256Hash hash) {
        checkArgument(chunks.get(0).isOpCode()); // P2SH scriptSig
        int numKeys = Script.decodeFromOpN(chunks.get(chunks.size() - 2).opcode);
        TransactionSignature signature = TransactionSignature.decodeFromNubits(signatureBytes, true);
        for (int i = 0 ; i < numKeys ; i++) {
    ...
    static int decodeFromOpN(int opcode) {
        checkArgument((opcode == ScriptOpCodes.OP_0 || opcode == ScriptOpCodes.OP_1NEGATE) || (opcode >= ScriptOpCodes.OP_1 && opcode <= ScriptOpCodes.OP_16), "decodeFromOpN called on non OP_N opcode");
     ...
     * logic is defined by <a href="https://github.com/nubits/bips/blob/master/bip-0016.mediawiki">BIP 16</a>.</p>
     *
     * <p>nubitsj does not support creation of P2SH transactions today. The goal of P2SH is to allow short addresses
    ...
    public boolean isOpReturn() {
        return chunks.size() == 2 && chunks.get(0).equalsOpCode(ScriptOpCodes.OP_RETURN);
    }

    /**
     * Exposes the script interpreter. Normally you should not use this directly, instead use
     * {@link com.nubits.nubitsj.core.TransactionInput#verify(com.nubits.nubitsj.core.TransactionOutput)} or
     * {@link com.nubits.nubitsj.script.Script#correctlySpends(com.nubits.nubitsj.core.Transaction, long, Script)}. This method
    ...
    public static void executeScript(@Nullable Transaction txContainingThis, long index,
        ...
        for (ScriptChunk chunk : script.chunks) {
            ...
            if (!chunk.isOpCode()) {
                ...
                if (opcode == ScriptOpCodes.OP_CAT || opcode == ScriptOpCodes.OP_SUBSTR || opcode == ScriptOpCodes.OP_LEFT || opcode == ScriptOpCodes.OP_RIGHT ||
                    opcode == ScriptOpCodes.OP_INVERT || opcode == ScriptOpCodes.OP_AND || opcode == ScriptOpCodes.OP_OR || opcode == ScriptOpCodes.OP_XOR ||
                    opcode == ScriptOpCodes.OP_2MUL || opcode == ScriptOpCodes.OP_2DIV || opcode == ScriptOpCodes.OP_MUL || opcode == ScriptOpCodes.OP_DIV ||
    ...
    private static void executeCheckSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
        ...
        try {
            TransactionSignature sig  = TransactionSignature.decodeFromNubits(sigBytes, false);
    ...
    private static int executeMultiSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
        ...
        while (sigs.size() > 0) {
            ...
            try {
                TransactionSignature sig = TransactionSignature.decodeFromNubits(sigs.getFirst(), false);
                Sha256Hash hash = txContainingThis.hashForSignature(index, connectedScript, (byte) sig.sighashFlags);
    ...
    public void correctlySpends(Transaction txContainingThis, long scriptSigIndex, Script scriptPubKey,
        ...
        try {
            txContainingThis = new Transaction(txContainingThis.getParams(), txContainingThis.nubitsSerialize());
