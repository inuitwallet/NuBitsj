    ...
    private void receiveVersionMessage(Protos.TwoWayChannelMessage msg) throws VerificationException {
        ...
        if (major != SERVER_MAJOR_VERSION) {
            error("This server needs protocol version " + SERVER_MAJOR_VERSION + " , client offered " + major,
                    Protos.Error.ErrorCode.NO_ACCEPTABLE_VERSION, PaymentChannelCloseException.CloseReason.NO_ACCEPTABLE_VERSION);
            return;
        }
            ...
            if (ackInfoFuture == null) {
                ...
                Futures.addCallback(ackInfoFuture, new FutureCallback<ByteString>() {
                    ...
                    public void onFailure(Throwable t) {
                        ...
                        error("Failed processing payment update", Protos.Error.ErrorCode.OTHER, PaymentChannelCloseException.CloseReason.UPDATE_PAYMENT_FAILED);
                    }
                });
            }
        }
            ...
            try {
                switch (msg.getType()) {
                    ...
                    default:
                        ...
                        error(errorText, Protos.Error.ErrorCode.SYNTAX_ERROR, PaymentChannelCloseException.CloseReason.REMOTE_SENT_INVALID_MESSAGE);
                }
            } catch (VerificationException e) {
        ...
        Futures.addCallback(state.close(), new FutureCallback<Transaction>() {
            ...
            public void onSuccess(Transaction result) {
                ...
                if (result != null) {
                    ...
                    // properly and so on.
                    msg.getSettlementBuilder().setTx(ByteString.copyFrom(result.nubitsSerialize()));
