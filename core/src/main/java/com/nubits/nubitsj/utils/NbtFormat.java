...
package com.nubits.nubitsj.utils;

import com.nubits.nubitsj.utils.NbtAutoFormat.Style;
import static com.nubits.nubitsj.utils.NbtAutoFormat.Style.*;
 ...
 * representations of Nubits monetary values.
 *
 ...
 * represents a Nubits monetary value.  Some of the features offered for doing this
 * are: <ol>
 ...
 *       {@link NbtFormat#format(Object)} method. 
 *   <li>Parse a value by passing a <code>String</code>-type
 *       representation of it to the {@link NbtFormat#parse(String)} method.</ol>
 *
 * <p>For example, depending on your locale, values might be formatted
 * and parsed as follows:
 *
 * <blockquote><pre>
 * NbtFormat f = NbtFormat.getInstance();
 * String c = f.format(Coin.COIN);                <strong>// "Nbt 1.00"</strong>
 * String k = f.format(Coin.COIN.multiply(1000)); <strong>// "Nbt 1,000.00"</strong>
 * String m = f.format(Coin.COIN.divide(1000));   <strong>// "mNbt 1.00"</strong>
 * Coin all = f.parseObject("M𝑵 21");             <strong>// All the money in the world</strong>
 * </pre></blockquote>
 *
 ...
 * be formatted, {@link NbtAutoFormat}, and another that formats any value in units of a
 * fixed, specified denomination, {@link NbtFixedFormat}.  
 *
 ...
 * values of {@link NbtAutoFormat.Style}.  There are two styles constants: {@link
 * NbtAutoFormat.Style#CODE} (the default), and {@link NbtAutoFormat.Style#SYMBOL}.  The
 * difference is that the <code>CODE</code> style uses an internationally-distinct currency
 * code, such as <code>"NBT"</code>, to indicate the units of denomination, while the
 * <code>SYMBOL</code> style uses a possibly-ambiguous currency symbol such as
 * <code>"𝑵"</code>.
 *
 * <p>The denomination used when formatting will be either nubits, millicoin
 ...
 * locale, a value of one nubits might be formatted as <pre>𝑵1.00</pre> where a value
 * exceeding that by one satoshi would be <pre>µ𝑵1,000,000.01</pre>
 *
 ...
 * increasing precision to convert the representation of a given quantity of nubitss into a
 * representation of the same value denominated in the formatter's units.  For example, a scale
 * value of <code>3</code> specifies a denomination of millinubitss, because to represent
 * <code>1.0000 NBT</code>.
 *
 * <h3>Construction</h3>
 *
 * There are two ways to obtain an instance of this class: <ol>
 *   <li>Use one of the factory methods; or
 *   <li>Use a {@link NbtFormat.Builder} object.</ol>
 *
 ...
 * methods of this abstract base class <code>NbtFormat</code>.  There
 ...
 * automatic-denominating <code>NbtAutoFormat</code> instance for your
 ...
 * <blockquote><pre>
 * NbtFormat f = NbtFormat.getInstance();
 * String s = f.format(Coin.COIN); <strong>// "Nbt 1.00"</strong>
 * </pre></blockquote>
 *
 ...
 * the {@link NbtFixedFormat} instance that is returned, and thus will
 * determine its denomination.  For example, if you want to format
 * values in units of micronubitss:
 *
 * <blockquote><pre>NbtFormat m = NbtFormat.getInstance(6);
 ...
 *
 * <blockquote><pre>NbtFormat milliFormat = NbtFormat.getInstance(MILLICOIN_SCALE);</pre></blockquote>
 *
 * <p>Alternatively, if the type of the first argument to
 * <code>getInstance()</code> is one of the enumerated values of the
 * {@link NbtAutoFormat.Style} type, either <code>CODE</code> or
 * <code>SYMBOL</code>, then you will get a {@link NbtAutoFormat}
 ...
 * differently the same one-nubits value:
 *
 * <blockquote><pre>
 * <strong>// Next line returns "1,00 Nbt"</strong>
 * NbtFormat.getInstance(Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "1,00 𝑵"</strong>
 * NbtFormat.getInstance(SYMBOL, Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "1.000,00"</strong>
 * NbtFormat.getMilliInstance(Locale.GERMANY).format(Coin.COIN);
 * <strong>// Next line returns "10.000,00"</strong>
 * NbtFormat.getInstance(4, Locale.GERMANY).format(Coin.COIN);
 * </pre></blockquote>
 *
 ...
 *
 * A new {@link NbtFormat.Builder} instance is returned by the {@link #builder()} method.  Such
 * an object has methods that set the configuration parameters of a <code>NbtFormat</code>
 * object.  Its {@link Builder#build()} method constructs and returns a <code>NbtFormat</code> instance
 ...
 *
 * <blockquote><pre>
 * NbtFormat.Builder builder = NbtFormat.builder();
 * builder.style(SYMBOL);
 * builder.symbol("B&#x5c;u20e6"); <strong>// unicode char "double vertical stroke overlay"</strong>
 * NbtFormat f = builder.build();
 * String out = f.format(COIN); <strong>// "𝑵1.00" depending on locale</strong>
 * </pre></blockquote>
 *
 ...
 *
 * <blockquote><pre>
 * NbtFormat f = NbtFormat.builder().style(CODE).code("Nbt").build();
 * String out = f.format(COIN); <strong>// "Nbt 1.00"</strong>
 * </pre></blockquote>
 *
 * <p>See the documentation of the {@link NbtFormat.Builder} class for details.
 *
 * <h3>Formatting</h3>
 *
 * <p>You format a Nubits monetary value by passing it to the {@link NbtFormat#format(Object)}
 * method.  This argument can be either a {@link com.nubits.nubitsj.core.Coin}-type object or a
 ...
 * number of nubitss.  A value having a fractional amount of satoshis is rounded to the
 ...
 * {@link NbtFixedFormat} also accept a variable-length sequence of additional <code>int</code>
 ...
 *
 * <blockquote><pre>
 * NbtFormat f = NbtFormat.getCoinInstance();
 ...
 * {@link NbtFixedFormat#REPEATING_PLACES}, {@link
 * NbtFixedFormat#REPEATING_DOUBLETS} and {@link
 * NbtFixedFormat#REPEATING_TRIPLETS}.  These signify repeating groups
 ...
 *
 * <blockquote><pre>
 * NbtFormat de = NbtFormat.getInstance(Locale.GERMANY);
 * FieldPosition currField = new FieldPosition(NumberFormat.Field.CURRENCY);
 * <strong>// next line formats the value as "987.654.321,23 µNbt"</strong>
 * String output = de.format(valueOf(98765432123L), new StringBuffer(), currField);
 * <strong>// next line sets variable currencyCode to "µNbt"</strong>
 ...
 *
 * <blockquote><pre>
 * NbtFixedFormat kilo = (NbtFixedFormat)NbtFormat(-3); <strong>// scale -3 for kilocoins</strong>
 * Coin value = Coin.parseCoin("1230");
 * <strong>// variable coded will be set to "kNbt 1.23"</strong>
 * String coded = kilo.code() + " " + kilo.format(value);
 * <strong>// variable symbolic will be set to "k𝑵1.23"</strong>
 * String symbolic = kilo.symbol() + kilo.format(value);
 * NbtFormat(4).code(); <strong>// unnamed denomination has no code; raises exception</strong>
 * </pre></blockquote>
 *
 ...
 *                SATOSHI};
 * NbtFormat f = NbtFormat.getCoinInstance(2, REPEATING_PLACES);
 ...
 * NbtFixedFormat} and {@link NbtAutoFormat} is analogous to the difference in formatting
 * behavior between instances of those classes.  Instances of {@link NbtAutoFormat} recognize
 ...
 * instances of {@link NbtFixedFormat} by default recognize no codes nor symbols, but rather
 ...
 * NbtFixedFormat} can be overridden by setting a parsing pattern that includes a currency sign
 * using the {@link NbtFormat.Builder#pattern()} method.
 *
 * <p>The {@link NbtAutoFormat#parse(String)}</code> method of {@link NbtAutoFormat} (and of
 ...
 * <code>NbtFormat</code> instance, on the other hand, whether or not the units indicator must
 * be separated by a space from the number is determined by the locale.  The {@link
 * NbtFormat#pattern()} method returns a representation of the pattern that
 * can be examined to determine whether a space must separate currency signs from numbers in
 * parsed <code>String</code>s.
 *
 * <p>When parsing, if the currency-units indicator is absent, then a {@link NbtAutoFormat}
 * instance will infer a denomination of nubitss while a {@link NbtFixedFormat} will infer the
 ...
 * @see          com.nubits.nubitsj.core.Coin
 */

public abstract class NbtFormat extends Format {
    ...
    /** The conventional international currency code for nubitss: "Nbt" */
    private static final String COIN_CODE = "NBT";
    /** The default currency symbols for nubitss */
    private static final String COIN_SYMBOL = "𝑵";
    /** An alternative currency symbol to use in locales where the default symbol is used for the national currency. */
    protected static final String COIN_SYMBOL_ALT = "𝑵";
    ...
    /* Scale is the number of decimal-places difference from same value in nubitss */
    /** A constant useful for specifying a denomination of nubitss, the <code>int</code> value
     *  <code>0</code>. */
    public static final int COIN_SCALE = 0;

    /** A constant useful for specifying a denomination of millinubitss, the <code>int</code>
     *  value <code>3</code>. */
    public static final int MILLICOIN_SCALE = 3;

    /** A constant useful for specifying a denomination of micronubitss, the <code>int</code>
     *  value <code>6</code>. */
    ...
    /**
     * <p>This class constructs new instances of {@link NbtFormat}, allowing for the
     * configuration of those instances before they are constructed.  After obtaining a
     * <code>Builder</code> object from the {@link NbtFormat#builder()} method, invoke the
     * necessary setter methods to obtain your desired configuration.  Finaly, the {@link
     * #build()} method returns a new <code>NbtFormat</code> object that has the specified
     * configuration.
     *
     * <p>All the setter methods override defaults.  Invoking <code>build()</code> without invoking any
     * of the setting methods is equivalent to invoking {@link NbtFormat#getInstance()} with no arguments.
     *
    ...
    public static class Builder {
        ...
        private enum Variant {
            AUTO { @Override NbtFormat newInstance(Builder b) {
                       return getInstance(b.style, b.locale, b.minimumFractionDigits);
                   }},
            FIXED,
            UNSET;
            NbtFormat newInstance(Builder b) {
        ...
        private NbtAutoFormat.Style style = NbtAutoFormat.Style.CODE;
        private int scale = 0;
        private String symbol = "",code = "",pattern = "",localizedPattern = "";

        private Builder() {}

        /** Specify the new <code>NbtFormat</code> is to be automatically-denominating.
         * The argument determines which of either codes or symbols the new <code>NbtFormat</code>
        ...
        public Builder style(NbtAutoFormat.Style val) {
         ...
         * appropriately for the context of generating {@link NbtAutoFormat} instances.
         *
         ...
         *  denominated in nubitss.  For example, a denomination of millinubitss is specified
         *  with a value of <code>3</code>.
         *
         * <p>The <code>NbtFormat</code> class provides appropriately named
         * <code>int</code>-type constants for the three common values, {@link NbtFormat#COIN_SCALE},
         * {@link NbtFormat#MILLICOIN_SCALE} {@link NbtFormat#MICROCOIN_SCALE}.
         *
         * <p>If neither this method nor {@link #style(NbtAutoFormat.Style)} is invoked on a
         * <code>Builder</code>, then the <code>NbtFormat</code> will default to a
         * fixed-denomination of nubitss, equivalent to invoking this method with an argument
         * of <code>0</code>. */
         ...
         *  class.
         *
         *  <p>The pattern is localized according to the locale of the <code>NbtFormat</code>
         *  instance, the symbols for which can be examined by inspecting the {@link
         *  java.text.DecimalFormatSymbols} object returned by {@link NbtFormat#symbols()}.
        ...
        /** Return a new {@link NbtFormat} instance.  The object returned will be configured according
         *  to the state of this <code>Builder</code> instance at the time this method is invoked. */
        public NbtFormat build() {
            NbtFormat f = variant.newInstance(this);
    ...
    protected NbtFormat(DecimalFormat numberFormat, int minDecimals, List<Integer> groups) {
     ...
     * currency code, for example "<code>Nbt</code>", to indicate that denomination.  The
     ...
     */
    public static NbtFormat getInstance() { return getInstance(defaultLocale()); }

    /**
     * Return a new auto-denominating instance that will indicate units using a currency
     * symbol, for example, <code>"𝑵"</code>.  Formatting and parsing will be done
     * according to the default locale.
     */
    public static NbtFormat getSymbolInstance() { return getSymbolInstance(defaultLocale()); }

    /**
     * Return a new auto-denominating instance that will indicate units using a currency
     * code, for example, <code>"Nbt"</code>.  Formatting and parsing will be done
     * according to the default locale.
     */
    public static NbtFormat getCodeInstance() { return getCodeInstance(defaultLocale()); }
     ...
     * <code>"𝑵"</code>.  The returned object will format the fraction-part of numbers using
     ...
     * locale.
     */
    public static NbtFormat getSymbolInstance(int fractionPlaces) {
     ...
     * <code>"Nbt"</code>.  The returned object will format the fraction-part of numbers using
     * the given number of decimal places, or fewer as necessary to avoid giving a place to
     * fractional satoshis.  Formatting and parsing will be done according to the default
     * locale.
     */
    public static NbtFormat getCodeInstance(int minDecimals) {
     ...
     * units using a currency code, for example, <code>"mNbt"</code>.
     */
    public static NbtFormat getInstance(Locale locale) { return getCodeInstance(locale); }
     ...
     * units using a currency code, for example, <code>"mNbt"</code>.
     */
    public static NbtFormat getCodeInstance(Locale locale) { return getInstance(NbtAutoFormat.Style.CODE, locale); }
     ...
     * <code>"mNbt"</code>.  The returned object will format the fraction-part of numbers using
     * the given number of decimal places, or fewer as necessary to avoid giving a place to
     * fractional satoshis.
     */
    public static NbtFormat getInstance(Locale locale, int minDecimals) {
     ...
     * fractional satoshis.
     */
    public static NbtFormat getCodeInstance(Locale locale, int minDecimals) {
	return getInstance(NbtAutoFormat.Style.CODE, locale, minDecimals);
    }

    /**
     * Return a new symbol-style auto-formatter for the given locale.  The returned object will
     * select denominational units based on each value being formatted, and will indicate those
     * units using a currency symbol, for example, <code>"µ𝑵"</code>.
     */
    public static NbtFormat getSymbolInstance(Locale locale) {
	return getInstance(NbtAutoFormat.Style.SYMBOL, locale);
    }
     ...
     * fractional satoshis.
     */
    public static NbtFormat getSymbolInstance(Locale locale, int fractionPlaces) {
	return getInstance(NbtAutoFormat.Style.SYMBOL, locale, fractionPlaces);
    }
     ...
     * <code>"𝑵"</code>, or code, such as <code>"mNbt"</code>, depending on the value of
     * the argument.  Formatting and parsing will be done according to the default locale.
     */
    public static NbtFormat getInstance(NbtAutoFormat.Style style) { return getInstance(style, defaultLocale()); }
     ...
     * using either a currency symbol, such as, <code>"𝑵"</code>, or code, such as
     * <code>"mNbt"</code>, depending on the value of the first argument.  The returned object
     ...
     */
    public static NbtFormat getInstance(NbtAutoFormat.Style style, int fractionPlaces) {
     ...
     * "<code>NBT</code>", or symbol, such as "<code>𝑵</code>", depending on the value
     ...
     */
    public static NbtFormat getInstance(NbtAutoFormat.Style style, Locale locale) {
     ...
     * such as <code>"NBT"</code>, or symbol, such as "<code>𝑵</code>",
     ...
     * fractional satoshis.
     */
    public static NbtFormat getInstance(NbtAutoFormat.Style style, Locale locale, int fractionPlaces) {
	return new NbtAutoFormat(locale, style, fractionPlaces);
    }
     ...
     */
    public static NbtFormat getCoinInstance() { return getCoinInstance(defaultLocale()); }
     ...
     */
    public static NbtFormat getCoinInstance(int minFractionPlaces, int... groups) {
     ...
     */
    public static NbtFormat getCoinInstance(Locale locale) {
        return getInstance(COIN_SCALE, locale, 2);
    }

    /**
     * Return a newly-constructed instance for the given locale that will format
     * values in terms of nubitss, with the given minimum number of fractional
     ...
     */
    public static NbtFormat getCoinInstance(Locale locale, int scale, int... groups) {
     ...
     */
    public static NbtFormat getInstance(int scale, int minDecimals, int... groups) {
     ...
     */
    public static NbtFormat getInstance(int scale) {
     ...
     */
    public static NbtFormat getInstance(int scale, Locale locale) {
     ...
     */
    public static NbtFormat getInstance(int scale, Locale locale, int minDecimals, int... groups) {
     ...
     */
    public static NbtFormat getInstance(int scale, Locale locale, int minDecimals, List<Integer> groups) {
        return new NbtFixedFormat(locale, scale, minDecimals, groups);
    }
    ...
    /**
     * Formats a nubits monetary value and returns an {@link java.text.AttributedCharacterIterator}.
    ...
    /**
     * Formats a nubits value as a number and possibly a units indicator and appends the
     ...
     * number of nubitss, rounded to the nearest satoshi as necessary.
     *
    ...
    /**
     * Formats a nubits value as a number and possibly a units indicator to a
     ...
     * <code>BigDecimal</code> is interpreted as that number of nubitss, rounded to the
     ...
     * is the size of the decimal-place shift between the given Nubits-value denominated in
     * nubitss and that same value as formatted.  A fixed-denomination formatter will ignore
     * the arguments.
     *
    ...
    /**
     * Takes a nubits monetary value that the client wants to format and returns the number of
    ...
    /**
     * Takes an object representing a nubits quantity of any type the
	...
	BigInteger satoshis;
        // the value might be nubitss or satoshis
    ...
    /**
      * Parse a <code>String</code> representation of a Nubits monetary value.  Returns a
      * {@link com.nubits.nubitsj.core.Coin} object that represents the parsed value.
      * @see java.text.NumberFormat */
    @Override
    ...
    private volatile String ci = "(" + COIN_SYMBOL + "|" + COIN_SYMBOL_ALT + "|𝑵|" + COIN_CODE + "|NBT)";
    ...
    /** Parse a <code>String</code> representation of a Nubits monetary value.  If this
     * object's pattern includes a currency sign, either symbol or code, as by default is true
     * for instances of {@link NbtAutoFormat} and false for instances of {@link
     * NbtFixedFormat}, then denominated (i.e., prefixed) currency signs in the parsed String
     ...
     * String, then the number is interpreted as a quatity of nubitss.
     * <p>If the pattern contains neither a currency symbol nor sign, then instances of {@link
     * NbtAutoFormat} will interpret the parsed number as a quantity of nubitss, and instances
     * of {@link NbtAutoFormat} will interpret the number as a quantity of that instance's
     * configured denomination, which can be ascertained by invoking the {@link
     * NbtFixedFormat#symbol()} or {@link NbtFixedFormat#code()} method.
     *
    ...
    /** Return the unprefixed currency symbol for nubitss configured for this object.  The
    ...
    /** Return the unprefixed international currency code for nubitss configured for this
    ...
    @Override public boolean equals(Object o) {
        ...
        if (!(o instanceof NbtFormat)) return false;
        NbtFormat other = (NbtFormat)o;
