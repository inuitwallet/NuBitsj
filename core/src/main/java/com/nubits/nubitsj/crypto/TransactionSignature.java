...
/**
 * A TransactionSignature wraps an {@link com.nubits.nubitsj.core.ECKey.ECDSASignature} and adds methods for handling
...
public class TransactionSignature extends ECKey.ECDSASignature {
     ...
     * and generally violates people's mental model of how Nubits should work, thus, non-canonical signatures are now
     ...
     * of the type used by Nubits we have to encode them using DER encoding, which is just a way to pack the two
     * components into a structure, and then we append a byte to the end for the sighash flags.
     */
    public byte[] encodeToNubits() {
        try {
     ...
     */
    public static TransactionSignature decodeFromNubits(byte[] bytes, boolean requireCanonical) throws VerificationException {
        // Nubits encoding is DER signature + sighash byte.
