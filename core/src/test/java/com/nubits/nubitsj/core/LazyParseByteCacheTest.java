...
import com.nubits.nubitsj.testing.FakeTxBuilder;
...
import static com.nubits.nubitsj.testing.FakeTxBuilder.createFakeBlock;
import static com.nubits.nubitsj.testing.FakeTxBuilder.createFakeTx;
import static org.junit.Assert.*;

public class LazyParseByteCacheTest {

    private final byte[] txMessage = Utils.HEX.withSeparator(" ", 2).decode(
            "e6 e8 e9 e5 74 78 00 00  00 00 00 00 00 00 00 00" +
            "7a 01 00 00 23 b0 39 fb" +
            ...
            "69 fc 74 88 ac 00 00 00  00 42");
    
    private final byte[] txMessagePart = Utils.HEX.withSeparator(" ", 2).decode(
            "3d de 3e e6 40 dd 37 02  20 6d cb bb 70 1a e3 28" +
            "1f dc 5b fe a7 75 ba 46  2d 34 6f d1 a7 c5 9b 38" +
            "bf 77 c4 12 64 07 a5 11  40 01 21 02 d6 bb 69 ba" +
            "37 bf 00 53 81 5c f0 2b  8b 09 df 01 81 17 8a 2f"); 
    ...
    public void setUp() throws Exception {
        ...
        TransactionOutput prevOut = new TransactionOutput(unitTestParams, prevTx, Coin.COIN, wallet.currentReceiveKey().toAddress(unitTestParams));
        prevTx.addOutput(prevOut);
        // Connect it.
        tx1.addInput(prevOut);
        
        Transaction tx2 = FakeTxBuilder.createFakeTx(unitTestParams, Coin.COIN,
                new ECKey().toAddress(unitTestParams));

        Block b1 = FakeTxBuilder.createFakeBlock(blockStore, tx1, tx2).block;

        NubitsSerializer bs = new NubitsSerializer(unitTestParams);
        
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        bs.serialize(tx1, bos);
        tx1BytesWithHeader = bos.toByteArray();
        tx1Bytes = tx1.nubitsSerialize();
        
        bos.reset();
        bs.serialize(tx2, bos);
        tx2BytesWithHeader = bos.toByteArray();
        tx2Bytes = tx2.nubitsSerialize();
        
        bos.reset();
        bs.serialize(b1, bos);
        b1BytesWithHeader = bos.toByteArray();
        b1Bytes = b1.nubitsSerialize();
    }
    ...
    public void testBlock(byte[] blockBytes, boolean isChild, boolean lazy, boolean retain) throws Exception {
        ...
        //message structure.
        NubitsSerializer bsRef = new NubitsSerializer(unitTestParams, false, false);
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        
        NubitsSerializer bs = new NubitsSerializer(unitTestParams, lazy, retain);
        Block b1;
        Block bRef;
        b1 = (Block) bs.deserialize(ByteBuffer.wrap(blockBytes));
        bRef = (Block) bsRef.deserialize(ByteBuffer.wrap(blockBytes));
        
        //verify our reference NubitsSerializer produces matching byte array.
        bos.reset();
    ...
    public void testTransaction(NetworkParameters params, byte[] txBytes, boolean isChild, boolean lazy, boolean retain) throws Exception {
        ...
        //reference serializer to produce comparison serialization output after changes to
        //message structure.
        NubitsSerializer bsRef = new NubitsSerializer(params, false, false);
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        
        NubitsSerializer bs = new NubitsSerializer(params, lazy, retain);
        Transaction t1;
        Transaction tRef;
    ...
    private void serDeser(NubitsSerializer bs, Message message, byte[] sourceBytes, byte[] containedBytes, byte[] containingBytes) throws Exception {
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
