...
public class FullBlockTestGenerator {
    // Used by NubitsdComparisonTool and AbstractFullPrunedBlockChainTest to create test cases
    ...
    public RuleList getBlocksToTest(boolean addSigExpensiveBlocks, boolean runLargeReorgs, File blockStorageFile) throws ScriptException, ProtocolException, IOException {
        ...
        final Script OP_TRUE_SCRIPT = new ScriptBuilder().op(ScriptOpCodes.OP_TRUE).build();
        final Script OP_NOP_SCRIPT = new ScriptBuilder().op(ScriptOpCodes.OP_NOP).build();
            ...
            public boolean add(Rule element) {
                if (outStream != null && element instanceof BlockAndValidity) {
                    try {
                        ...
                        byte[] block = ((BlockAndValidity)element).block.nubitsSerialize();
        ...
        spendableOutputs.offer(new TransactionOutPointWithValue(
                ...
                Coin.valueOf(50, 0), chainHead.getTransactions().get(0).getOutputs().get(0).getScriptPubKey()));
        ...
        Block b15 = createNextBlock(b13, chainHeadHeight + 6, out5, null);
        {
            ...
            Arrays.fill(outputScript, (byte) ScriptOpCodes.OP_CHECKSIG);
        ...
        // Extend the b24 chain to make sure nubitsd isn't accepting b24
        ...
        // Extend the b26 chain to make sure nubitsd isn't accepting b26
        ...
        // Extend the b28 chain to make sure nubitsd isn't accepting b28
        ...
        Block b31 = createNextBlock(b30, chainHeadHeight + 9, out8, null);
        {
            ...
            Arrays.fill(outputScript, (byte) ScriptOpCodes.OP_CHECKMULTISIG);
            tx.addOutput(new TransactionOutput(params, tx, SATOSHI, outputScript));
            addOnlyInputToTransaction(tx, new TransactionOutPointWithValue(
        ...
        Block b32 = createNextBlock(b31, chainHeadHeight + 10, out9, null);
        {
            ...
            for (int i = 0; i < (Block.MAX_BLOCK_SIGOPS - sigOps)%20; i++)
                outputScript[i] = (byte) ScriptOpCodes.OP_CHECKSIG;
            tx.addOutput(new TransactionOutput(params, tx, SATOSHI, outputScript));
            addOnlyInputToTransaction(tx, new TransactionOutPointWithValue(
        ...
        Block b33 = createNextBlock(b31, chainHeadHeight + 10, out9, null);
        {
            ...
            Transaction tx = new Transaction(params);
            byte[] outputScript = new byte[(Block.MAX_BLOCK_SIGOPS - sigOps)/20];
            Arrays.fill(outputScript, (byte) ScriptOpCodes.OP_CHECKMULTISIGVERIFY);
            tx.addOutput(new TransactionOutput(params, tx, SATOSHI, outputScript));
            addOnlyInputToTransaction(tx, new TransactionOutPointWithValue(
        ...
        Block b35 = createNextBlock(b33, chainHeadHeight + 11, out10, null);
        {
            ...
            Transaction tx = new Transaction(params);
            byte[] outputScript = new byte[Block.MAX_BLOCK_SIGOPS - sigOps];
            Arrays.fill(outputScript, (byte) ScriptOpCodes.OP_CHECKSIGVERIFY);
            tx.addOutput(new TransactionOutput(params, tx, SATOSHI, outputScript));
            addOnlyInputToTransaction(tx, new TransactionOutPointWithValue(
        ...
        Block b39 = createNextBlock(b35, chainHeadHeight + 12, null, null);
        {
            ...
            try {
                ...
                p2shScriptPubKey.write(ScriptOpCodes.OP_2DUP);
                p2shScriptPubKey.write(ScriptOpCodes.OP_CHECKSIGVERIFY);
                p2shScriptPubKey.write(ScriptOpCodes.OP_2DUP);
                p2shScriptPubKey.write(ScriptOpCodes.OP_CHECKSIGVERIFY);
                p2shScriptPubKey.write(ScriptOpCodes.OP_2DUP);
                p2shScriptPubKey.write(ScriptOpCodes.OP_CHECKSIGVERIFY);
                ...
                p2shScriptPubKey.write(ScriptOpCodes.OP_2DUP);
                p2shScriptPubKey.write(ScriptOpCodes.OP_CHECKSIGVERIFY);
                p2shScriptPubKey.write(ScriptOpCodes.OP_CHECKSIG);
            } catch (IOException e) {
            ...
            scriptPubKey.write(ScriptOpCodes.OP_HASH160);
            try {
                Script.writeBytes(scriptPubKey, scriptHash);
            } catch (IOException e) {
                throw new RuntimeException(e);  // Cannot happen.
            }
            scriptPubKey.write(ScriptOpCodes.OP_EQUAL);
            ...
            TransactionOutPoint lastOutPoint;
            {
                ...
                tx.addOutput(new TransactionOutput(params, tx, lastOutputValue, new byte[]{ScriptOpCodes.OP_1}));
            ...
            while (b39.getMessageSize() < Block.MAX_BLOCK_SIZE)
            {
                ...
                tx.addOutput(new TransactionOutput(params, tx, SATOSHI, scriptPubKey.toByteArray()));
                tx.addOutput(new TransactionOutput(params, tx, lastOutputValue, new byte[]{ScriptOpCodes.OP_1}));
                tx.addInput(new TransactionInput(params, tx, new byte[]{ScriptOpCodes.OP_1}, lastOutPoint));
                lastOutPoint = new TransactionOutPoint(params, 1, tx.getHash());
        ...
        Block b40 = createNextBlock(b39, chainHeadHeight + 13, out12, null);
        {
            ...
            for (int i = 1; i <= numTxes; i++) {
                Transaction tx = new Transaction(params);
                tx.addOutput(new TransactionOutput(params, tx, SATOSHI, new byte[] {ScriptOpCodes.OP_1}));
                tx.addInput(new TransactionInput(params, tx, new byte[]{ScriptOpCodes.OP_1}, lastOutPoint));
                ...
                if (scriptSig == null) {
                    ...
                    Sha256Hash hash = tx.hashForSignature(1, b39p2shScriptPubKey, Transaction.SigHash.SINGLE, false);

                    // Sign input
                    try {
                        ByteArrayOutputStream bos = new UnsafeByteArrayOutputStream(73);
                        bos.write(coinbaseOutKey.sign(hash).encodeToDER());
                        bos.write(Transaction.SigHash.SINGLE.ordinal() + 1);
                        ...
                        Script.writeBytes(scriptSigBos, new byte[] {(byte) ScriptOpCodes.OP_CHECKSIG});
            ...
            Arrays.fill(scriptPubKey, (byte) ScriptOpCodes.OP_CHECKSIG);
            ...
            b41 = createNextBlock(b39, chainHeadHeight + 13, out12, null);
            {
                ...
                for (int i = 1; i <= numTxes; i++) {
                    ...
                    tx.addOutput(new TransactionOutput(params, tx, Coin
                            .SATOSHI, new byte[] {ScriptOpCodes.OP_1}));
                    tx.addInput(new TransactionInput(params, tx,
                            new byte[] {ScriptOpCodes.OP_1}, lastOutPoint));
                    ...
                    if (scriptSig == null) {
                        ...
                        Sha256Hash hash = tx.hashForSignature(1,
                                b39p2shScriptPubKey, Transaction.SigHash.SINGLE, false);
                        ...
                        try {
                            ...
                            Script.writeBytes(scriptSigBos,
                                    new byte[] { (byte) ScriptOpCodes.OP_CHECKSIG});
        ...
        byte[] outScriptBytes = ScriptBuilder.createOutputScript(ECKey.fromPublicOnly(coinbaseOutKeyPubKey)).getProgram();
        {
            ...
            t.addOutput(new TransactionOutput(params, t, ZERO, new byte[] {ScriptOpCodes.OP_PUSHDATA1 - 1 }));
            t.addOutput(new TransactionOutput(params, t, SATOSHI, outScriptBytes));
            // Spendable output
            t.addOutput(new TransactionOutput(params, t, ZERO, new byte[] {ScriptOpCodes.OP_1}));
        ...
        Block b56;
        try {
            b56 = new Block(params, b57.nubitsSerialize());
        ...
        Block b58 = createNextBlock(b57, chainHeadHeight + 18, out17, null);
        {
            ...
            tx.addInput(new TransactionInput(params, tx, new byte[] {ScriptOpCodes.OP_1},
        ...
        Block b64;
        {
            ...
            for (Transaction transaction : b64Created.getTransactions())
                transaction.nubitsSerialize(stream);
            ...
            checkState(Arrays.equals(stream.toByteArray(), b64.nubitsSerialize()));
        ...
        Block b71 = new Block(params, b72.nubitsSerialize());
        ...
        Block b73 = createNextBlock(b72, chainHeadHeight + 23, out22, null);
        {
            ...
            outputScript[Block.MAX_BLOCK_SIGOPS - sigOps] = ScriptOpCodes.OP_PUSHDATA4;
        ...
        Block b74 = createNextBlock(b72, chainHeadHeight + 23, out22, null);
        {
            ...
            outputScript[Block.MAX_BLOCK_SIGOPS - sigOps + 1] = ScriptOpCodes.OP_PUSHDATA4;
    ...
    private Block createNextBlock(Block baseBlock, int nextBlockHeight, @Nullable TransactionOutPointWithValue prevOut,
        ...
        Coin coinbaseValue = (prevOut != null ? prevOut.value.subtract(SATOSHI) : ZERO)
        ...
        if (prevOut != null) {
            ...
            // Spendable output
            t.addOutput(new TransactionOutput(params, t, ZERO, new byte[] {ScriptOpCodes.OP_1, uniquenessCounter++}));
    ...
    private void addOnlyInputToTransaction(Transaction t, TransactionOutPointWithValue prevOut, long sequence) throws ScriptException {
        ...
        if (prevOut.scriptPubKey.getChunks().get(0).equalsOpCode(ScriptOpCodes.OP_TRUE)) {
            input.setScriptSig(new ScriptBuilder().op(ScriptOpCodes.OP_1).build());
        } else {
            // Sign input
            checkState(prevOut.scriptPubKey.isSentToRawPubKey());
            Sha256Hash hash = t.hashForSignature(0, prevOut.scriptPubKey, Transaction.SigHash.ALL, false);
            input.setScriptSig(ScriptBuilder.createInputScript(
                new TransactionSignature(coinbaseOutKey.sign(hash), Transaction.SigHash.ALL, false))
            );
        }
    }
}
