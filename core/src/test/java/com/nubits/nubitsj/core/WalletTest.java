...
import com.nubits.nubitsj.signers.StatelessTransactionSigner;
import com.nubits.nubitsj.signers.TransactionSigner;
...
import static com.nubits.nubitsj.core.Coin.*;
import static com.nubits.nubitsj.core.Utils.HEX;
import static com.nubits.nubitsj.testing.FakeTxBuilder.*;
...
public class WalletTest extends TestWithWallet {
    ...
    // For nubits we shall send two cents, one cent for fee and another to meet minimum output requirement
    Coin TWO_CENTS = CENT.multiply(2);
    ...
    private Transaction cleanupCommon(Address destination) throws Exception {
        receiveATransaction(wallet, myAddress); // Got 1 coin
        ...
        assertEquals(valueOf(0, 59), wallet.getBalance(Wallet.BalanceType.ESTIMATED)); // Should be 1 coin minus .51 plus .1 = .59
    ...
    private void basicSpendingCommon(Wallet wallet, Address toAddress, Address destination, boolean testEncryption) throws Exception {
        ...
        // will attach a small fee. Because the Nubits protocol makes it difficult to determine the fee of an
        ...
        try {
            ...
            assertEquals(valueOf(9, 1), e.missing); // Nu: Miss nine coins and cent fee
        }
    ...
    private void spendUnconfirmedChange(Wallet wallet, Transaction t2, KeyParameter aesKey) throws Exception {
        ...
        Wallet.SendRequest req = Wallet.SendRequest.to(new ECKey().toAddress(params), valueOf(0, 47));
        req.aesKey = aesKey;
        req.shuffleOutputs = false;
        wallet.completeTx(req);
        ...
        FakeTxBuilder.BlockPair bp = FakeTxBuilder.createFakeBlock(blockStore, t2, t3);
    ...
    public void balance() throws Exception {
        ...
        spend.params = MainNetParams.get(); // Nu: Use normal selectable logic
        wallet.commitTx(spend);
        assertEquals(1, wallet.getPoolSize(WalletTransaction.Pool.PENDING));

        // Available and estimated balances should not be the same. We don't check the exact available balance here
        // because it depends on the coin selection algorithm.
        assertEquals(valueOf(4, 49), wallet.getBalance(Wallet.BalanceType.ESTIMATED)); // Nu: minus fee
        assertFalse(wallet.getBalance(Wallet.BalanceType.AVAILABLE).equals(
                    wallet.getBalance(Wallet.BalanceType.ESTIMATED)));

        // Now confirm the transaction by including it into a block.
        StoredBlock b3 = FakeTxBuilder.createFakeBlock(blockStore, spend).storedBlock;
        wallet.receiveFromBlock(spend, b3, BlockChain.NewBlockType.BEST_CHAIN, 0);

        // Change is confirmed. We started with 5.50 so we should have 4.49 left.
        Coin v4 = valueOf(4, 49);
    ...
    public void blockChainCatchup() throws Exception {
        ...
        Threading.waitForUserCode();
        assertEquals(Coin.valueOf(0, 89), wallet.getBalance());
        ...
        assertEquals(Coin.valueOf(0, 89), bigints[3]);
        ...
        assertEquals(Coin.valueOf(0, 78), wallet.getBalance());
        Threading.waitForUserCode();
        FakeTxBuilder.BlockPair b4 = FakeTxBuilder.createFakeBlock(blockStore);
        confTxns.clear();
    ...
    public void balances() throws Exception {
        ...
        Transaction send1 = wallet.createSend(new ECKey().toAddress(params), valueOf(0, 10));
        // Reserialize.
        Transaction send2 = new Transaction(params, send1.nubitsSerialize());
        assertEquals(nanos, send2.getValueSentFromMe(wallet));
        assertEquals(ZERO.subtract(valueOf(0, 11)), send2.getValue(wallet)); // Nu: Cent fee
    }
    ...
    public void isConsistent_duplicates() throws Exception {
        ...
        Transaction txClone = new Transaction(params, tx.nubitsSerialize());
        try {
    ...
    public void bounce() throws Exception {
        ...
        inbound2.addOutput(new TransactionOutput(params, inbound2, coinHalf.subtract(CENT), myAddress)); // Nu: Less a cent
        ...
        assertEquals(coin1.subtract(TWO_CENTS), wallet.getBalance()); // Nu: Two cents loss in two transactions
    }
    ...
    public void doubleSpendUnspendsOtherInputs() throws Exception {
        ...
        // Nu: Make sure main net is used to make coins unselectable
        ...
        send2 = new Transaction(params, send2.nubitsSerialize());
        ...
        assertEquals(valueOf(1, 99), wallet.getBalance()); // Nu: minus fee
        assertTrue(wallet.isConsistent());
    }
    ...
    public void doubleSpends() throws Exception {
        ...
        byte[] buf = send1.nubitsSerialize();
        buf[47] = 0;  // Break the signature: nubitsj won't check in SPV mode and this is easier than other mutations
        // Nu: we used 47 thus adding 4 for the timestamp
        send1 = new Transaction(params, buf);
        wallet.commitTx(send2);
        wallet.allowSpendingUnconfirmedTransactions();
        assertEquals(value.subtract(CENT), wallet.getBalance(Wallet.BalanceType.ESTIMATED)); // Nu: minus cent fee
        // Now spend the change. This transaction should die permanently when the mutant appears in the chain.
        Transaction send3 = checkNotNull(wallet.createSend(address, value.subtract(TWO_CENTS))); // Nu: minus the first fee and then another required fee
        ...
        // Double spend!
        sendMoneyToWallet(send1, AbstractBlockChain.NewBlockType.BEST_CHAIN);
        // Back to having one coin minus fee
        assertEquals(value.subtract(CENT), wallet.getBalance());
    ...
    public void pending1() throws Exception {
        ...
        final Transaction t1 = FakeTxBuilder.createFakeTx(params, nanos, myAddress);
        ...
        final Transaction t1Copy = new Transaction(params, t1.nubitsSerialize());
    ...
    public void pending2() throws Exception {
        ...
        Transaction t2 = wallet.createSend(new ECKey().toAddress(params), valueOf(0, 50));
    ...
    public void spendToSameWallet() throws Exception {
        ...
        // Nu: We will use MainNetParams so to make pending transaction unselectable
    	outbound1.params = MainNetParams.get();
        wallet.commitTx(outbound1);
        // We should have a zero available balance before the next block.
        assertEquals(ZERO, wallet.getBalance());
        sendMoneyToWallet(outbound1, AbstractBlockChain.NewBlockType.BEST_CHAIN);
        // We should have a balance of 1 Nbt after the block is received.
        assertEquals(coin1.subtract(CENT), wallet.getBalance()); // Nu: minus cent fee
    }
    ...
    public void lastBlockSeen() throws Exception {
        ...
        Transaction t1 = FakeTxBuilder.createFakeTx(params, v1, myAddress);
        Transaction t2 = FakeTxBuilder.createFakeTx(params, v2, myAddress);
        Transaction t3 = FakeTxBuilder.createFakeTx(params, v3, myAddress);
    ...
    public void pubkeyOnlyScripts() throws Exception {
        ...
        Transaction t1 = FakeTxBuilder.createFakeTx(params, value, key1);
        if (wallet.isPendingTransactionRelevant(t1))
            wallet.receivePending(t1, null);
        // TX should have been seen as relevant.
        assertEquals(value, wallet.getBalance(Wallet.BalanceType.ESTIMATED));
        assertEquals(ZERO, wallet.getBalance(Wallet.BalanceType.AVAILABLE));
        Block b1 = FakeTxBuilder.createFakeBlock(blockStore, t1).block;
        chain.add(b1);
        ...
        Transaction t2 = wallet.createSend(new ECKey().toAddress(params), value.subtract(CENT)); // Nu: minus fee
        assertNotNull(t2);
    ...
    public void watchingScripts() throws Exception {
        ...
        Coin value = valueOf(5, 0);
        Transaction t1 = FakeTxBuilder.createFakeTx(params, value, watchedAddress);
    ...
    public void watchingScriptsConfirmed() throws Exception {
        ...
        StoredBlock b3 = FakeTxBuilder.createFakeBlock(blockStore, t1).storedBlock;
    ...
    public void watchingScriptsSentFrom() throws Exception {
        ...
        StoredBlock b1 = FakeTxBuilder.createFakeBlock(blockStore, t1).storedBlock;
    ...
    public void watchingScriptsBloomFilter() throws Exception {
        ...
        assertFalse(wallet.getBloomFilter(1e-12).contains(outPoint.nubitsSerialize()));

        wallet.receiveFromBlock(t1, b1, BlockChain.NewBlockType.BEST_CHAIN, 0);
        assertTrue(wallet.getBloomFilter(1e-12).contains(outPoint.nubitsSerialize()));
    }
    ...
    public void marriedKeychainBloomFilter() throws Exception {
        ...
        assertFalse(wallet.getBloomFilter(0.001).contains(outPoint.nubitsSerialize()));

        wallet.receiveFromBlock(t1, b1, BlockChain.NewBlockType.BEST_CHAIN, 0);
        assertTrue(wallet.getBloomFilter(0.001).contains(outPoint.nubitsSerialize()));
    }

    @Test
    public void autosaveImmediate() throws Exception {
        // Test that the wallet will save itself automatically when it changes.
        File f = File.createTempFile("nubitsj-unit-test", null);
    ...
    public void autosaveDelayed() throws Exception {
        ...
        Block b0 = FakeTxBuilder.createFakeBlock(blockStore).block;
        chain.add(b0);
        Sha256Hash hash3 = Sha256Hash.hashFileContents(f);
        ...
        Block b3 = FakeTxBuilder.createFakeBlock(blockStore, t2).block;
        chain.add(b3);
    ...
    public void replayWhilstPending() throws Exception {
        ...
        Coin value = COIN;
        Transaction tx1 = FakeTxBuilder.createFakeTx(params, value, myAddress);
        Transaction tx2 = new Transaction(params);
        ...
        FakeTxBuilder.BlockPair bp = FakeTxBuilder.createFakeBlock(blockStore, tx1);
    ...
    public void encryptionDecryptionBasic() throws Exception {
        assertEquals(Protos.Wallet.EncryptionType.ENCRYPTED_SCRYPT_AES, encryptedWallet.getEncryptionType());
    ...
    public void encryptionDecryptionBadPassword() throws Exception {
        ...
        assertTrue("Wallet is not an encrypted wallet", encryptedWallet.getEncryptionType() == Protos.Wallet.EncryptionType.ENCRYPTED_SCRYPT_AES);
    ...
    public void encryptionDecryptionCheckExceptions() throws Exception {
        ...
        assertTrue("Wallet is not an encrypted wallet.2", encryptedWallet.getEncryptionType() == Protos.Wallet.EncryptionType.ENCRYPTED_SCRYPT_AES);
        ...
        assertTrue("Wallet is not an encrypted wallet.3", encryptedWallet.getEncryptionType() == Protos.Wallet.EncryptionType.ENCRYPTED_SCRYPT_AES);
    }
    ...
    public void ageMattersDuringSelection() throws Exception {
        ...
        for (int i = 0; i < ITERATIONS; i++) {
            txns[i] = sendMoneyToWallet(COIN.add(CENT), AbstractBlockChain.NewBlockType.BEST_CHAIN); // Nu: Give room for fee
        }
    ...
    public void sendMultipleCentsTest() throws Exception {
        ...
        Transaction tx = new Transaction(params);
        Address notMyAddr = new ECKey().toAddress(params);
        tx.addOutput(COIN.CENT, notMyAddr);
        tx.addOutput(COIN.CENT, notMyAddr);
        tx.addOutput(COIN.CENT, notMyAddr);
        tx.addOutput(COIN.CENT, notMyAddr);
        SendRequest request = Wallet.SendRequest.forTx(tx);
    ...
    public void feeSolverAndCoinSelectionTest() throws Exception {
        ...
        StoredBlock block = new StoredBlock(FakeTxBuilder.makeSolvedTestBlock(blockStore, notMyAddr), BigInteger.ONE, 1);
        Transaction tx1 = createFakeTx(params, CENT, myAddress);
        ...
        Transaction tx3 = createFakeTx(params, CENT.multiply(10), myAddress);
        wallet.receiveFromBlock(tx3, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 2);

        // Not allowed to send too small output
        try {
            wallet.createSend(notMyAddr, Transaction.MIN_OUTPUT_VALUE.subtract(SATOSHI));
            fail();
        } catch (Wallet.TooSmallOutput e) {
            // Expected.
        }

        // Spend it 
        SendRequest req = SendRequest.to(notMyAddr, CENT.multiply(11)); // Nu: minus cent fee
        assertNotNull(wallet.sendCoinsOffline(req));
        assertEquals(ZERO, wallet.getBalance());

        // Add coin
        block = new StoredBlock(FakeTxBuilder.makeSolvedTestBlock(blockStore, notMyAddr), BigInteger.ONE, 1);
        ...
        // Nu: Check minimum fee is enforced
        ...
        // Nu: Check less than cent change is disposed of.
        SendRequest request5 = SendRequest.to(notMyAddr, Coin.COIN.subtract(TWO_CENTS.subtract(SATOSHI)));
        wallet.completeTx(request5);
        assertEquals(Transaction.REFERENCE_DEFAULT_MIN_TX_FEE.add(CENT.subtract(SATOSHI)), request5.tx.getFee());
        Transaction spend5 = request5.tx;
        // If we would have a change output < 0.01 it is disposed of
        assertEquals(1, spend5.getOutputs().size());
        assertEquals(spend5.getOutput(0).getValue(), Coin.COIN.subtract(TWO_CENTS.subtract(SATOSHI)));

        SendRequest request6 = SendRequest.to(notMyAddr, Coin.COIN.subtract(TWO_CENTS)); // Nu: Extra cent for fee
        wallet.completeTx(request6);
        assertEquals(CENT, request6.tx.getFee());
        Transaction spend6 = request6.tx;
        // ...but not if change output == 0.01
        assertEquals(2, spend6.getOutputs().size());
        // We optimize for priority, so the output selected should be the largest one
        assertEquals(Coin.COIN.subtract(CENT), spend6.getOutput(0).getValue().add(spend6.getOutput(1).getValue())); // Nu: minus cent fee

        // Nu: All balance minus fee gives no change
        SendRequest request8 = SendRequest.to(notMyAddr, COIN.subtract(CENT));
        ...
        // Remove the coin from our wallet and add two cents
        wallet.commitTx(spend8);
        Transaction tx5 = FakeTxBuilder.createFakeTx(params, TWO_CENTS, myAddress);
        wallet.receiveFromBlock(tx5, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 0);
        assertEquals(TWO_CENTS, wallet.getBalance());
        ...
        assertTrue(tx5.getOutput(0).isMine(wallet) && tx5.getOutput(0).isAvailableForSpending() && tx5.getConfidence().getDepthInBlocks() == 50);
        ...
        // Nu: Used 1.00 coin, now 0.9 due to fee
        assertTrue(spend12.getOutputs().size() == 2 && spend12.getOutput(0).getValue().add(spend12.getOutput(1).getValue()).equals(COIN.subtract(CENT)));

        wallet.notifyNewBestBlock(block);
        assertTrue(tx5.getOutput(0).isMine(wallet) && tx5.getOutput(0).isAvailableForSpending() && tx5.getConfidence().getDepthInBlocks() == 51);
        assertTrue(tx6.getOutput(0).isMine(wallet) && tx6.getOutput(0).isAvailableForSpending() && tx6.getConfidence().getDepthInBlocks() == 1);
        // Now tx5 has slightly higher coin*depth tx6...
        ...
        block = new StoredBlock(FakeTxBuilder.makeSolvedTestBlock(blockStore, notMyAddr), BigInteger.ONE, 1);
        wallet.notifyNewBestBlock(block);
        assertTrue(tx5.getOutput(0).isMine(wallet) && tx5.getOutput(0).isAvailableForSpending() && tx5.getConfidence().getDepthInBlocks() == 52);
        ...
        // Nu: Used 1.00 coin, now 0.9 due to fee
        assertTrue(spend14.getOutputs().size() == 2 && spend14.getOutput(0).getValue().add(spend14.getOutput(1).getValue()).equals(COIN.subtract(CENT)));
    ...
    public void basicCategoryStepTest() throws Exception {
        ...
        int i = 0;
        while (i <= 50) {
            Transaction tx = FakeTxBuilder.createFakeTxWithChangeAddress(params, CENT, myAddress, notMyAddr);
    ...
    public void testCategory2WithChange() throws Exception {
        ...
        while (i <= CENT.divide(Transaction.REFERENCE_DEFAULT_MIN_TX_FEE.multiply(10))) {
            Transaction tx = FakeTxBuilder.createFakeTxWithChangeAddress(params, Transaction.REFERENCE_DEFAULT_MIN_TX_FEE.multiply(10), myAddress, notMyAddr);
            tx.getInput(0).setSequenceNumber(i++); // Keep every transaction unique
            wallet.receiveFromBlock(tx, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, i);
        }
    ...
    public void lowerThanDefaultFee() throws InsufficientMoneyException, IOException {
        ...
        req.feePerKb = fee;
        wallet.completeTx(req);
        assertEquals(Transaction.REFERENCE_DEFAULT_MIN_TX_FEE, req.tx.getFee());
        wallet.commitTx(req.tx);
    ...
    public void feePerKbCategoryJumpTest() throws Exception {
        ...
        // Nu: With inputs of Coin plus cent we can pay 1.00 with no change
        
        // Create a transaction who's max size could be up to 1000 (if signatures were maximum size)
        SendRequest request1 = makeJustOneKbReq(notMyAddr, COIN);
        wallet.completeTx(request1);
        assertEquals(Transaction.REFERENCE_DEFAULT_MIN_TX_FEE, request1.tx.getFee());
    ...
    public void testCompleteTxWithExistingInputs() throws Exception {
        ...
        wallet.receiveFromBlock(tx2, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 1);
        Transaction tx3 = FakeTxBuilder.createFakeTx(params, TWO_CENTS, myAddress);
        wallet.receiveFromBlock(tx3, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 2);
        ...
        assertEquals(COIN.subtract(CENT.multiply(2)), request1.tx.getOutput(1).getValue()); // Nu: cent fee
        ...
        // However, if there is no connected output, we will grab a COIN output anyway and add the CENT to fee, in addition to the normal CENT fee
        SendRequest request3 = SendRequest.to(notMyAddr, CENT);
        ...
        wallet.completeTx(request3);
        assertEquals(2, request3.tx.getInputs().size()); // Coin and two cents
        assertEquals(2, request3.tx.getOutputs().size()); // Coin minus cent and cent
        assertEquals(CENT, request3.tx.getOutput(0).getValue());
        assertEquals(COIN.subtract(TWO_CENTS), request3.tx.getOutput(1).getValue());
    ...
    public void testEmptyWallet() throws Exception {
        ...
        // Add exactly 0.01
        StoredBlock block = new StoredBlock(FakeTxBuilder.makeSolvedTestBlock(blockStore, outputKey), BigInteger.ONE, 1);
        Transaction tx = FakeTxBuilder.createFakeTx(params, TWO_CENTS, myAddress); // Nu: Need an extra cent for fee
        wallet.receiveFromBlock(tx, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 0);
        SendRequest request = SendRequest.emptyWallet(outputKey);
        wallet.completeTx(request);
        assertEquals(CENT, request.tx.getFee());
        wallet.commitTx(request.tx);
        assertEquals(ZERO, wallet.getBalance());
        assertEquals(CENT, request.tx.getOutput(0).getValue());

        // Add 2 confirmed cents and 2 unconfirmed cents. Verify only 2 cents is emptied because of the coin selection
        // policies that are in use by default.
        block = new StoredBlock(FakeTxBuilder.makeSolvedTestBlock(blockStore, outputKey), BigInteger.ONE, 1);
        tx = FakeTxBuilder.createFakeTx(params, TWO_CENTS, myAddress);
        wallet.receiveFromBlock(tx, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 0);
        tx = FakeTxBuilder.createFakeTx(params, TWO_CENTS, myAddress);
        ...
        StoredBlock block3 = new StoredBlock(block.getHeader().createNextBlock(outputKey), BigInteger.ONE, 3);
        Coin outputValue = Transaction.MIN_OUTPUT_VALUE.add(Transaction.REFERENCE_DEFAULT_MIN_TX_FEE).subtract(SATOSHI);
        tx = FakeTxBuilder.createFakeTx(params, outputValue, myAddress);
    ...
    public void keyRotationRandom() throws Exception {
        ...
        sendMoneyToWallet(wallet, TWO_CENTS, toAddress, AbstractBlockChain.NewBlockType.BEST_CHAIN);
        ...
        sendMoneyToWallet(wallet, TWO_CENTS, key1.toAddress(params), AbstractBlockChain.NewBlockType.BEST_CHAIN);
        wallet.doMaintenance(null, true); 
        ...
        assertEquals(CENT, tx.getOutput(0).getValue()); // Nu: Two cents minus fee to one cent remaining.
        ...
        final Address address = new ECKey().toAddress(params);
        wallet.sendCoins(broadcaster, address, wallet.getBalance().subtract(CENT));
    ...
    public void keyRotationHD2() throws Exception {
        ...
        sendMoneyToWallet(wallet, TWO_CENTS, badWatchingKey.toAddress(params), AbstractBlockChain.NewBlockType.BEST_CHAIN);
        ...
        assertEquals("BFHAnUa1XQEC5dVhushF9pncAtu6RkMpxf", usedKey.toAddress(params).toString());
    ...
    public void completeTxPartiallySignedWithDummySigs() throws Exception {
        byte[] dummySig = TransactionSignature.dummy().encodeToNubits();
    ...
    public void riskAnalysis() throws Exception {
        ...
        StoredBlock block = FakeTxBuilder.createFakeBlock(blockStore, tx).storedBlock;
    ...
    public void sendRequestExchangeRate() throws Exception {
        receiveATransactionAmount(wallet, myAddress, COIN.add(CENT));
    ...
    public void sendRequestMemo() throws Exception {
        receiveATransaction(wallet, myAddress);
        SendRequest sendRequest = SendRequest.to(myAddress, Coin.COIN.subtract(CENT)); // Nu: minus fee
