...
public class NubitsSerializerTest {
    private final byte[] addrMessage = Utils.HEX.decode("e6e8e9e56164647200000000000000001f000000" +
    ...
    public void testLazyParsing()  throws Exception {
        NubitsSerializer bs = new NubitsSerializer(MainNetParams.get(), true, false);
    ...
    private void testCachedParsing(boolean lazy)  throws Exception {
        NubitsSerializer bs = new NubitsSerializer(MainNetParams.get(), lazy, true);
    ...
    public void testHeaders1() throws Exception {
        ...
        HeadersMessage hm = (HeadersMessage) bs.deserialize(ByteBuffer.wrap(Utils.HEX.decode("e6e8e9e5686561" +
        ...
        Assert.assertEquals(Utils.HEX.encode(block.getMerkleRoot().getBytes()),
    ...
    public void testHeaders2() throws Exception {
        ...
        HeadersMessage hm = (HeadersMessage) bs.deserialize(ByteBuffer.wrap(Utils.HEX.decode("e6e8e9e56865616465" +
    ...
    @Test
    public void testNubitsPacketHeader() {
        try {
            new NubitsSerializer.NubitsPacketHeader(ByteBuffer.wrap(new byte[]{0}));
            fail();
        } catch (BufferUnderflowException e) {
        }

        // Message with a Message size which is 1 too big, in little endian format.
        byte[] wrongMessageLength = Utils.HEX.decode("000000000000000000000000010000020000000000");
        try {
            new NubitsSerializer.NubitsPacketHeader(ByteBuffer.wrap(wrongMessageLength));
            fail();
    ...
    public void testSeekPastMagicBytes() {
        ...
        byte[] brokenMessage = Utils.HEX.decode("000000");
        try {
            new NubitsSerializer(MainNetParams.get()).seekPastMagicBytes(ByteBuffer.wrap(brokenMessage));
            fail();
        } catch (BufferUnderflowException e) {
            // expected
        }
